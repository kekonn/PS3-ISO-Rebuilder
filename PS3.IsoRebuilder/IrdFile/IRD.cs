...
using System.Text;
using PS3.IsoRebuilder.IrdFile;
    ...
    public class IRD
    {
        public struct Region
        {
            ...
            public uint End
            {
                get
                {
                    checked
                    {
                        return (uint)((long)checked(Start + Length) - 1L);
                    }
                }
            }
            ...
            public Region(int region, uint s, uint l, bool plain, byte[] hash)
            {
                ...
                checked
                {
                    ...
                    Start = s;
                    Length = (uint)((long)checked(l - s) + 1L);
                    isPlain = plain;
                    SourceHash = hash;
                }
            }
        ...
        public bool Parse(string path)
        {
            ...
            checked
            {
                ...
                var crc = cRC.GetCrc32(ref stream, position);
                if (this.crc == crc)
                {
                    valid = true;
                }
                ...
                var binaryReader = new BinaryReader(header, Encoding.ASCII);
                binaryReader.BaseStream.Seek(0L, SeekOrigin.Begin);
                var num5 = (uint)((long)Swap(binaryReader.ReadUInt32()) * 2L - 1);
                binaryReader.BaseStream.Seek(4L, SeekOrigin.Current);
                var s = Swap(binaryReader.ReadUInt32());
                var flag = true;
                for (var num6 = 0u; num6 < num5; num6 = (uint)((long)num6 + 1L))
                {
                    var num7 = Swap(binaryReader.ReadUInt32());
                    if (flag)
                    {
                        var regions = Regions;
                        var key2 = (int)num6;
                        var value2 = new Region((int)num6, s, num7, flag, RegionHashes[(int)num6]);
                        regions.Add(key2, value2);
                    }
                    else
                    {
                        var regions2 = Regions;
                        var key3 = (int)num6;
                        var value2 = new Region((int)num6, s, num7, flag, RegionHashes[(int)num6]);
                ...
                try
                {
                    ...
                    disksize = (ulong)((long)isoheader.VolumeDescriptor.VolumeSpaceSize * isoheader.Blocksize);
                }
                catch (Exception ex)
                {
        ...
        public Stream Uncompress(BinaryReader input)
        {
            checked
            {
                var array = new byte[(int)((long)input.ReadUInt32() - 1L) + 1];
